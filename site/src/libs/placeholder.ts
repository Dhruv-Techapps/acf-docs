...
export function getPlaceholder(userOptions: Partial<PlaceholderOptions>): Placeholder {
  ...
  const placeholderLabel = showText || showTitle ? `${showTitle ? title : ''}${showTitle && showText ? ': ' : ''}${showText ? text : ''}` : undefined;
...
export function replacePlaceholdersInHtml(html: string) {
  return html.replace(placeholderRegex, (match) => {
    ...
    if (document.children.length > 1 || !placeholderElement || placeholderElement.type !== htmlparser2.ElementType.Tag || placeholderElement.name !== 'Placeholder') {
...
function getPlaceholderSrc(showTitle: boolean, showText: boolean, { background, color, text, title }: PlaceholderOptions) {
