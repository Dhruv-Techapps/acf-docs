...
toc: true
aliases:
  - '/docs/4.x/action-value/function/'
---

import { getDocsRelativePath } from '@libs/path';

<Callout type="info">
  Now run browser default API functions or custom func from extension directly. This feature is still under development and we will provide better location to run command from action. Run one or more
  than one command by separating them using `;` <br />
