...
execFile('java', ['-version'], (error, stdout, stderr) => {
  ...
  const ignores = [
    'Bad value “100%” for attribute “width” on element “video”.*',
    ...
    'The .*aria-labelledby.* attribute must not be specified on any .*div.* element unless the element has a .*role.* value other than.*',
    'The heading .*follows the heading .*, skipping [0-9]+ heading level[s]?.'
  ].join('|');

  const args = ['-jar', `"${vnu}"`, '--asciiquotes', '--skip-non-html', '--Werror', `--filterpattern "${ignores}"`, '_site/', 'js/tests/'];
